...
public class Problem05 {
    public static void main(String[] args) {
        if (args.length != 2) {
            System.err.println("Usage: java Problem05 <Word to remove> <File name>");
            System.exit(-1);
        }
        ...
        try (var input = new Scanner(fileToProcess);
             var output = new PrintWriter(tempFile)) {
            while (input.hasNext()) {
                output.println(input.nextLine().replaceAll(args[1], ""));
            }
        } catch (IOException ignored) {
